...
totaldiatom <- phyto_1m[c(84,86,87,88,89,90,91), "Diatom_Sum"]
totaldiatom
watertemps <- waterconditions[c(104,106,107,108,111,112,113), "Water_Temp_1m_C"]
watertemps
...
datesID <- c("20230606", "20230613", "20230620", "20230626", "20230705", "20230711", "20230718")
POSR_REAL$ID_yyyymmdd <- datesID
juliandates <- c(23157, 23164, 23171, 23177, 23186, 23192, 23199)
...
barplotvalues <- matrix(data = c(POSR_REAL$Male, POSR_REAL$Female, POSR_REAL$Hermaphrodite), nrow = 3, ncol = 7, byrow = TRUE)
...
regions <- c("Male", "Female", "Hermaphrodite")
barplotdates = c("6/6", "6/13", "6/20", "6/26", "7/5", "7/11", "7/18")
...
#instal tidyverse
long_barplot <- gather(data = barplotdata_2, key = "day", value = "count", 1:7) # key = description for columns you have & value = description for what is in the cells of your data frame
long_barplot

# change date to be factor
long_barplot$day <- factor(long_barplot$day, levels = c("6/6", "6/13", "6/20", "6/26", "7/5", "7/11", "7/18"))
